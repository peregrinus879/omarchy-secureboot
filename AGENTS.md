...
- **limine-entry-tool 1.36.0+**: 1.36.0 added `/etc/boot/hooks/post.d/89-warn-missing-file-hashes`, which warns on Limine 12 when Secure Boot and `ENABLE_ENROLL_LIMINE_CONFIG` are active without `ENABLE_VERIFICATION=yes`; that is this repo's deliberate model, the warning is harmless (quieted upstream in 1.37.0), and the hook sorts before `zzz-omarchy-secureboot-sign`. 1.37.1 reads sbctl state via JSON. The `10-limine-reset-enroll`/`90-limine-enroll-config` hooks are unchanged through 1.37.1.
