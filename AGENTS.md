...
- **Limine 12 compatibility**: Limine 12 can enforce BLAKE2B hashes on non-EFI loaded paths when Secure Boot and config checksum enrollment are both active. Omarchy currently boots UKIs via `protocol: efi`, which is exempt because firmware Secure Boot verifies EFI binaries. Limine 12 also changes interface color options from 0-7 indexes to `RRGGBB` values. Do not add automatic path-hash rewriting unless Omarchy moves to non-EFI loaded paths or explicitly enables that model. `status` should warn proactively about non-EFI path hashes and color values instead. Limine 12.5.2 keeps CONFIG.md and Secure Boot semantics identical to 12.5.1 but rebuilds every UEFI binary (PE `.bss` change), so the normal re-sign hooks fire on that package upgrade.
...
- **Omarchy Quattro (4.0) transition**: Quattro converts Omarchy into pacman packages (`omarchy`, `omarchy-settings`) under `/usr/share/omarchy` and moves Limine settings into `/etc/limine-entry-tool.d/{omarchy-defaults,omarchy-uki}.conf` drop-ins, while `/etc/default/limine` remains a read layer (the upgrade appends `root=` there). `CUSTOM_UKI_NAME="omarchy"`, the UKI path, the boot package set, and `/etc/boot/hooks/{pre.d,post.d}` all survive; Quattro itself ships no sbctl or Secure Boot logic, so this repo remains the gap-filler. `omarchy-upgrade-to-quattro` mutates boot state (seds `limine.conf` colors, deletes the legacy `<machine-id>_linux.efi` UKI leaving a stale sbctl entry for the cleanup path, removes shadow `limine.conf` files, reruns `limine-mkinitcpio`): run `status` and `sign` after the upgrade and before reboot. Quattro's `00-omarchy-update-guard.hook` blocks direct `pacman -Syu` unless `OMARCHY_UPDATE_PACMAN=1`; it sorts before `zz-*` and leaves the hook ordering invariant intact.
- **limine-entry-tool 1.37.x**: 1.36.0's post-hook can warn when Secure Boot and `ENABLE_ENROLL_LIMINE_CONFIG` are active without `ENABLE_VERIFICATION=yes`; that is this repo's deliberate model and the warning is harmless (quieted upstream in 1.37.0). 1.37.x adds `/etc/boot/hooks/post.d/89-warn-missing-file-hashes`, which sorts before `zzz-omarchy-secureboot-sign`, and 1.37.1 reads sbctl state via JSON. The `10-limine-reset-enroll`/`90-limine-enroll-config` hooks are unchanged through 1.37.1.
