...
- Current Limine packages provide `/etc/boot/hooks/pre.d/10-limine-reset-enroll`, `/etc/boot/hooks/post.d/89-warn-missing-file-hashes`, and `/etc/boot/hooks/post.d/90-limine-enroll-config`. The warning hook sorts before `zzz-omasecboot-sign` and stays silent for EFI-exempt UKI builds on current versions. limine-entry-tool reads sbctl state through JSON.
