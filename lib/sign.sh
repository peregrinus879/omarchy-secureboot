...
# Disable Limine's Blake2b hash verification in /etc/default/limine.
# With Secure Boot active, UEFI firmware signature verification supersedes
# Limine's hash check. Hashes become stale after signing and cause boot warnings.
# Returns 0 if setting was changed (caller should run limine-update),
# 1 if already disabled or config not found.
disable_limine_verification() {
  [[ -f "$LIMINE_DEFAULT_CONF" ]] || return 1

  if grep -q '^ENABLE_VERIFICATION=no' "$LIMINE_DEFAULT_CONF" 2>/dev/null; then
    qpass "Limine hash verification already disabled"
    return 1
  fi

  if grep -q '^ENABLE_VERIFICATION=' "$LIMINE_DEFAULT_CONF" 2>/dev/null; then
    sed -i 's/^ENABLE_VERIFICATION=.*/ENABLE_VERIFICATION=no/' "$LIMINE_DEFAULT_CONF"
  else
    echo "ENABLE_VERIFICATION=no" >> "$LIMINE_DEFAULT_CONF"
  fi
  qact "Disabled Limine hash verification (Secure Boot supersedes it)"
  return 0
}

# Strip any remaining Blake2b hashes from limine.conf path directives.
# Safety net for entries not regenerated by limine-update (e.g., snapshots).
strip_limine_hashes() {
  [[ -f "$LIMINE_CONF" ]] || return 0
  grep -qE '#[0-9a-f]{64,}$' "$LIMINE_CONF" || return 0
  sed -i 's/#[0-9a-f]\{64,\}$//' "$LIMINE_CONF"
  qact "Stripped stale Blake2b hashes from limine.conf"
}
